...
When you change a TailorDB table definition, the SDK can generate a migration that captures the diff and, for breaking changes, runs a data transformation script during `tailor deploy`. See the [TailorDB Migrations guide](tailordb-migration) for the full workflow, configuration, supported change types, team coordination, and troubleshooting.
