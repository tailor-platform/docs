...
- [TailorDB](services/tailordb) - Database schema definition
- [Resolver](services/resolver) - Custom GraphQL resolvers
- [Executor](services/executor) - Event-driven handlers
- [Workflow](services/workflow) - Job orchestration
- [Auth](services/auth) - Authentication and authorization
- [IdP](services/idp) - Built-in identity provider
- [Static Website](services/staticwebsite) - Static file hosting
- [Secret Manager](services/secret) - Secure credential storage

To deploy the same config to multiple workspaces with per-environment values, see [Multi-Environment Configuration](multi-environment).
...
**CORS**: Specify CORS settings as an array. You can also include Static Website URL references (e.g. `website.url`) in this array; see [Static Website](services/staticwebsite).
...
Configure the Built-in IdP service using `defineIdp()`. See [IdP](services/idp) for full documentation.
...
Configure Auth service using `defineAuth()`. See [Auth](services/auth) for full documentation.
...
Configure static website hosting using `defineStaticWebSite()`. See [Static Website](services/staticwebsite) for full documentation.
...
Configure secrets using `defineSecretManager()`. See [Secret Manager](services/secret) for full documentation.
...
Move the value to [Secret Manager](services/secret) to fix this. Detection recognizes the credential formats published by common providers, such as Slack, GitHub and AWS.
